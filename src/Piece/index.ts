...
import { applyGeometry, type Geometry, type GeometryContext } from "../Geometry";
...
export class Piece {
    ...
    constructor(id: string, xbetza: string, ctx: GeometryContext, geometry: Geometry = SquareRectGeometry) {
        this.id = id;
        this.xbetza = xbetza;
        this.geometry = geometry;
        const normalized = parseXBetza(xbetza);
        this.atoms = normalized.map(atom =>
            applyGeometry(atom, geometry, ctx)
        );
    }
}
